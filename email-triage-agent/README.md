...
- **SECURITY_ALERT**: Security alerts, CVEs, breach notifications, suspicious login attempts
- **PURCHASE**: Purchase confirmations, invoices, receipts
- **SHIPPING**: Package tracking, delivery notifications

## Camel Components Used

- **camel-openai**: AI-powered email classification and summarization, with structured output (JSON Schema). Works with OpenAI and any OpenAI-compatible API (Ollama, vLLM, LiteLLM)
...
- **application.properties**: LLM model config, Gmail OAuth2 credentials template, and external dependencies
...
camel.component.openai.apiKey=<YOUR_OPENAI_API_KEY>
```
...
The triage step uses the `camel-openai` component's `jsonSchema` parameter to enforce structured output. Instead of relying on prompt instructions to produce valid JSON, the schema (`email-triage-result.schema.json`) is passed directly to the LLM, which guarantees the response conforms to the defined structure:
